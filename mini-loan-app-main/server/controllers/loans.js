...
export const createLoan = async (req, res) => {
  try {
    ...
    // Check if amount and terms are valid numbers
    const parsedAmount = parseInt(amount);
    const parsedTerms = parseInt(terms);

    if (isNaN(parsedAmount) || isNaN(parsedTerms) || parsedAmount <= 0 || parsedTerms <= 0) {
      throw new Error("Invalid amount or terms");
    }
    ...
    const repaymentAmount = Math.round(parsedAmount / parsedTerms);
    const today = new Date();
    for (let i = 0; i < parsedTerms; i++) {
